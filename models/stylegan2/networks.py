...
class MappingNetwork(torch.nn.Module):
    ...
    def forward(self, z, c=512, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False):
        # Embed, normalize, and concat inputs.
        x = None
        # print('shape of z: ', len(z))
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, conditions = None,force_fp32=False, fused_modconv=None, **layer_kwargs):
        ...
        if self.in_channels == 0:
            ...
            if conditions is not None and conditions[0].size()[2] == x.size()[2] and conditions[0].size()[3] == x.size()[3]:
                x = x*(1+conditions[0]) + conditions[1]
                # print('resnet add conditions!!!!!!!!')
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, conditions =None, **block_kwargs):
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f'b{res}')
            x, img = block(x, img, cur_ws, conditions, **block_kwargs)
        return img
...
class Generator(torch.nn.Module):
    ...
    def mean_latent(self, n_latent):
        latent_in = torch.randn(
            n_latent, self.w_dim, device='cpu'
        )
        # print('latent in: ', latent_in.size())
        latent = self.mapping(latent_in).mean(0, keepdim=True)
        # print('mean  latent: ', latent.size())
        return latent

    def get_latent(self, input):
        return self.mapping(input)

    def forward(self, z, conditions=None,c=512, truncation_psi=1, input_is_latent = False,truncation_cutoff=None, **synthesis_kwargs):
        if not input_is_latent:
            ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
            return ws,z
        else:
            img = self.synthesis(z,conditions, **synthesis_kwargs)
            return img, z
